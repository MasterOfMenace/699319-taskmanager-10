...
import {getTodayTasks} from "../utils/filter";
...
const getTasksByDateRange = (tasks, dateFrom, dateTo) => {
  return tasks.filter((task) => {
    ...
    return dueDate >= dateFrom <= dateTo;
  });
};
...
const renderColorsChart = (ctx, tasks) => {
  const colors = tasks.map((it) => it.color).filter(getUniqItems);
...
const renderDaysChart = (ctx, tasks, dateFrom, dateTo) => {
  const days = calculateBetweenDates(dateFrom, dateTo);

  const tasksCount = days.map((day) => {
...
const renderTagsChart = (ctx, tasks) => {
...
export default class StatisticsComponent extends AbstractSmartComponent {
  ...
  _renderCharts() {
    ...
    const dayCtx = element.querySelector(`.statistic__days`);
    const colorCtx = element.querySelector(`.statistic__colors`);
    const tagCtx = element.querySelector(`.statistic__tags`);

    this._resetCharts();

    this._daysChart = renderDaysChart(dayCtx, this._tasks.getTasks(), this._dateFrom, this._dateTo);
    this._colorsChart = renderColorsChart(colorCtx, this._tasks.getTasks(), this._dateFrom, this._dateTo);
    this._tagsChart = renderTagsChart(tagCtx, this._tasks.getTasks(), this._dateFrom, this._dateTo);
  }
